...
func _ready():
	...
	if Functions.player_position != Vector2.ZERO:
		global_position = Functions.player_position
...
func _physics_process(_delta: float) -> void:
	...
	if direction:
		...
		ray_cast.target_position = Vector2(direction.x, ray_y).normalized() * 20
...
func get_basic_input():
	...
	if Input.is_action_just_pressed("action"):
		...
		if not target:
			# If hitting nothing, use tools
			tool_state_machine.travel(Data.TOOL_STATE_ANIMATIONS[current_tool])
			$Animation/AnimationTree.set("parameters/ToolOneShot/request", AnimationNodeOneShot.ONE_SHOT_REQUEST_FIRE)
		else:
			# 1. Check for NPCs (Grandma)
			if target.has_method("interact"):
				target.interact(self)
			
			if target.has_method("start_dialog"):
				target.start_dialog()
			
			# 2. Check for Quest Items
			if target.is_in_group("QuestItem"):
				print("Im interacting with an item")
				# todo: check if item is needed for quest and 
				# todo: remove item
